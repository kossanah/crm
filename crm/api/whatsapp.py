...
def before_insert(doc, method):
    """Set reference fields for incoming messages before insert."""
...
def after_insert(doc, method):
    """Publish realtime event immediately after message is created."""
...
def on_update(doc, method):
    """Publish realtime event when message is updated."""
    if doc.reference_doctype and doc.reference_name:
        frappe.publish_realtime(
            "whatsapp_message",
            {
...
def get_whatsapp_messages(reference_doctype, reference_name):
    ...
    for reaction_message in reaction_messages:
        ...
        reacted_message = next(
            (m for m in messages if m["message_id"] ==
             reaction_message["reply_to_message_id"]),
            None,
        )
