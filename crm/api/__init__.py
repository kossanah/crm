...
try:
    ...
    # Fallback implementation for Frappe v15
    def get_modules_from_all_apps_for_user(user=None):
        """Backward compatible implementation for Frappe v15"""
        user = user or frappe.session.user
        all_modules = []

        # Get all modules from all apps
        for app in frappe.get_installed_apps():
            modules = frappe.get_all("Module Def", filters={"app_name": app}, fields=[
                                     "module_name", "app_name as app"])
            all_modules.extend(modules)

        # Filter blocked modules
        if user and user != "Administrator":
            try:
                global_blocked = frappe.get_doc(
                    "User", "Administrator").get_blocked_modules()
                user_blocked = frappe.get_doc(
                    "User", user).get_blocked_modules()
                blocked_modules = global_blocked + user_blocked
                all_modules = [m for m in all_modules if m.get(
                    "module_name") not in blocked_modules]
            except Exception:
                pass  # If method doesn't exist, return all modules
...
def accept_invitation(key: str | None = None):
    ...
    result = frappe.db.get_all("CRM Invitation", filters={
                               "key": key}, pluck="name")
    if not result:
        frappe.throw("Invalid or expired key")
...
def invite_by_email(emails: str, role: str):
    ...
    for email in to_invite:
        frappe.get_doc(doctype="CRM Invitation", email=email,
