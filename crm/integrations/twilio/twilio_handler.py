...
class Twilio:
    ...
    def __init__(self, settings):
        ...
        self.enable_byoc = settings.enable_byoc
        self.byoc_trunk_sid = settings.byoc_trunk_sid if settings.enable_byoc else None
    ...
    def generate_voice_access_token(self, identity: str, ttl=60 * 60):
        ...
        token = AccessToken(self.account_sid, self.api_key,
                            self.api_secret, identity=identity, ttl=ttl)
    ...
    def get_twilio_client(self):
        ...
        if not twilio_settings.enabled:
            frappe.throw(
                _("Please enable twilio settings before making a call."))

        auth_token = get_decrypted_password(
            "CRM Twilio Settings", "CRM Twilio Settings", "auth_token")
        client = TwilioClient(twilio_settings.account_sid, auth_token)

        return client

    def generate_twilio_dial_response(self, from_number: str, to_number: str):
        """Generates voice call instructions with BYOC support"""
        resp = VoiceResponse()
        dial_kwargs = {
            'caller_id': from_number,
            'record': self.settings.record_calls,
        ...
        # Add BYOC if enabled
        if self.enable_byoc and self.byoc_trunk_sid:
            dial_kwargs['byoc'] = self.byoc_trunk_sid
    ...
    def make_outbound_call(self, from_number: str, to_number: str, webhook_url: str):
        """Make outbound call with BYOC support"""
        call_params = {
            'to': to_number,
            'from': from_number,
            'url': webhook_url,
        ...
        # Add BYOC parameter if enabled
        if self.enable_byoc and self.byoc_trunk_sid:
            call_params['byoc'] = self.byoc_trunk_sid

        return self.twilio_client.calls.create(**call_params)
...
def get_the_call_attender(owners, caller=None):
    ...
    if len(current_loggedin_users) > 1 and caller:
        deal_owner = frappe.db.get_value(
            "CRM Deal", {"mobile_no": caller}, "deal_owner")
        if not deal_owner:
            deal_owner = frappe.db.get_value(
                "CRM Lead", {"mobile_no": caller,
                             "converted": False}, "lead_owner"
            )
