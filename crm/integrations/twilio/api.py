...
def create_call_log(call_details: TwilioCallDetails):
    ...
    call_log = frappe.get_doc(
        {**details, "doctype": "CRM Call Log", "telephony_medium": "Twilio"})

    # link call log with lead/deal
    contact_number = details.get("from") if details.get(
        "type") == "Incoming" else details.get("to")
...
def update_recording_info(**kwargs):
    try:
        ...
        frappe.db.set_value("CRM Call Log", call_sid,
...
@frappe.whitelist()
def is_byoc_enabled():
    """Check if BYOC is enabled"""
    settings = frappe.get_doc("CRM Twilio Settings")
    return settings.enabled and settings.enable_byoc


@frappe.whitelist()
def get_byoc_trunk_info():
    """Get BYOC trunk information"""
    settings = frappe.get_doc("CRM Twilio Settings")
    if not (settings.enabled and settings.enable_byoc):
        return {}

    return {
        "trunk_sid": settings.byoc_trunk_sid,
        "enabled": True
    }
...
def get_datetime_from_timestamp(timestamp):
    ...
    datetime_utc_tz = datetime.strptime(
        datetime_utc_tz_str, "%Y-%m-%d %H:%M:%S%z")
